...
- [ ] **GET /users/{id}**
  ...
  - **작업**:
    - 사용자 ID 기반으로 지급 내역 반환 (`RewardHistoryDTO` 활용)
    - 지급된 리워드 개수, 지급 요청일, 지급 이유 포함
    - 총 지급 내역 개수 (`totalCount`) 반환
    - 예외 처리 (사용자 미존재 시 `404 NOT FOUND`)
...
- [ ] **GET /users/{id}**
  ...
  - **작업**:
    - 작가 ID 기반으로 지급 내역 반환
    - 작가 여부 검증 (`UserRole.AUTHOR` 확인)
    - 지급된 리워드 개수, 지급 이유, 작품 정보 (`WorkActivityDTO`) 포함
    - 총 지급 내역 개수 (`totalCount`) 반환
    - 예외 처리 (작가 미존재 시 `404 NOT FOUND`, 일반 사용자가 접근 시 `403 FORBIDDEN`)
...
- [ ] **공통 예외 처리 적용**
  - **설명**: 모든 API에서 일관된 예외 처리를 제공
  - **작업**:
    - `400 BAD REQUEST` → 잘못된 요청 데이터
    - `403 FORBIDDEN` → 권한 부족 (예: 일반 사용자가 작가 API 접근)
    - `404 NOT FOUND` → 리소스 미존재 (사용자, 작가, 리워드 등)
    - `500 INTERNAL SERVER ERROR` → 서버 내부 오류 발생 시 처리
